...
jQuery.webshims.register(
    'form-number-date-ui', function ($, webshims, window, document, undefined, options) {
        "use strict";
        ...
        var replaceInputUI = function (context, elem) {
            $('input', context).add(elem.filter('input')).each(
                function () {
        ...
        var lazySetDate = function (elem, date) {
            ...
            $.data(
                elem[0], 'setDateLazyTimer', setTimeout(
                    function () {
                        elem.datepicker('setDate', date);
            ...
            if(Modernizr.formvalidation) {
                orig.on(
                    'firstinvalid', function (e) {
                        ...
                        orig.off('invalid.replacedwidgetbubble').on(
                            'invalid.replacedwidgetbubble', function (evt) {
                ...
                $(orig[0].form).on(
                    'reset', function (e) {
                        if(e.originalEvent && !e.isDefaultPrevented()) {
                            setTimeout(
                                function () {
                                    orig.prop('value', orig.prop('value'));}, 0
                            );
                        }
                    }
                );
        ...
        if(!modernizrInputTypes['date'] /*||!modernizrInputTypes['datetime-local']*/ || options.replaceUI) {
            ...
            var configureDatePicker = function (elem, datePicker, change, _wrapper) {
                ...
                .on(
                    {
                        focusin: function () {
                              resetFocusHandler();
                            data.dpDiv.unbind('mousedown.webshimsmousedownhandler').bind(
                                'mousedown.webshimsmousedownhandler', function () {
                                    stopFocusout = true;
                                }
                            );
                        },
                ...
                ['disabled', 'min', 'max', 'value', 'step', 'data-placeholder'].forEach(
                    function (name) {
        ...
        if (!modernizrInputTypes.range || options.replaceUI) {
            replaceInputUI.range = function (elem) {
                ...
                ['disabled', 'min', 'max', 'step', 'value'].forEach(
                    function (name) {
        ...
        $.each(
            ['disabled', 'min', 'max', 'value', 'step', 'data-placeholder'], function (i, attr) {
                webshims.onNodeNamesPropertyModify(
                    'input', attr, function (val) {
            ...
            webshims.activeLang(
                {
                    ...
                    callback: function (langObj) {
                        ...
                        if(datepickerCFG.dateFormat && options.datepicker.dateFormat != datepickerCFG.dateFormat ) {
                            ...
                            .attr(
                                'data-placeholder', function (i, val) {
                                    return val;
                                }
                            );
                        }
        ...
        (function () {
            ...
            webshims.addReady(
                function (context, contextElem) {
                    ...
                    if(options.stepArrows) {
                        $('input', context).add(contextElem.filter('input')).each(
                            function () {
                                 var type = $.prop(this, 'type');
                                ...
                                .on(
                                    {
                                        'selectstart dragstart': function () {
                                            return false;},
                                ...
                                .on(
                                    ($.browser.msie) ? 'keydown' : 'keypress', function (e) {
                                        var step = stepKeys[e.keyCode];
                                        if(step) {
                                              uiEvents.step(step);
                                              return false;
        ...
        webshims.addReady(
            function (context, elem) {
                $(document).on(
                    'jquery-uiReady.initinputui input-widgetsReady.initinputui', function (e) {
